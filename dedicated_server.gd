...
func _server_apply_input_command(peer_id: int, cmd: Dictionary, cmd_time_sec: float) -> void:
	...
	if bool(s.get("alive", true)):
		var allow_move := server_phase == "live" or server_phase == "waiting"
		if allow_move:
		...
		if weapon_id == "ak":
			...
			if (ak_auto and (mask & INPUT_FIRE_HELD) != 0) or ((not ak_auto) and (mask & INPUT_FIRE_JUST) != 0):
				_server_process_hitscan_shot(peer_id, weapon_id, s, cmd_time_sec)
...
func _server_broadcast_shot_fx(peer_id: int, weapon: String, aim_angle: float, length_px: float) -> void:
	if multiplayer.multiplayer_peer == null:
		return
	var stream := _make_packet_stream()
	...
	stream.put_u16(_angle_to_u16(aim_angle))
...
func _server_process_hitscan_shot(sender: int, weapon: String, shooter_state: Dictionary, shot_time_sec: float) -> void:
	if server_phase != "live" and server_phase != "waiting":
		return
	...
	var dir := Vector2.RIGHT.rotated(float(shooter_state.get("aim", 0.0)))
	...
	_server_broadcast_shot_fx(sender, weapon, float(shooter_state.get("aim", 0.0)), hit_len)
	_server_mark_dirty()
...
func net_submit_shot(payload: Dictionary) -> void:
	...
	_server_process_hitscan_shot(sender, str(s.get("weapon", "glock")), s, Time.get_ticks_msec() / 1000.0)
...
@rpc("authority", "call_local")
func net_server_runtime_flags(_is_admin_flag: bool, _team_code: String, _practice_mode: bool, _practice_bot_count: int, _inf_ammo: bool, _inf_nades: bool, _inf_money: bool) -> void:
	pass
