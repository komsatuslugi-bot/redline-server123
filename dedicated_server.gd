...
const ONLINE_ROUND_BUY_SEC: float = 13.0
...
func _server_consume_recoil_offset_deg(state: Dictionary, weapon_id: String, shot_time_sec: float, is_moving: bool) -> float:
	...
	var pattern_offset := _server_get_recoil_pattern_offset_deg(profile, shot_index)
	var noise_offset := _server_get_recoil_noise_deg(profile, shot_index, is_moving)
...
func _server_apply_input_command(peer_id: int, cmd: Dictionary, cmd_time_sec: float) -> void:
	...
	if bool(s.get("alive", true)):
		...
		if allow_move:
			...
			s["x"] = float(s.get("x", 0.0)) + dir.x * SERVER_MOVE_SPEED_PVP * NET_USERCMD_DT_SEC
			s["y"] = float(s.get("y", 0.0)) + dir.y * SERVER_MOVE_SPEED_PVP * NET_USERCMD_DT_SEC
		if weapon_id == "ak":
...
func _server_apply_buy(peer_id: int, action_id: String) -> void:
	...
	match action_id:
		"armor":
			if (free_buy or money_now >= ARMOR_PRICE_PVP) and int(s.get("armor", 0)) < 100:
				if not free_buy:
