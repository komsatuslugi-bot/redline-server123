...
const ARMOR_PRICE_PVP: int = 900
const LIGHT_ARMOR_PRICE_PVP: int = 500
const HEAVY_ARMOR_PRICE_PVP: int = 900
const LIGHT_ARMOR_POINTS_PVP: int = 35
const HEAVY_ARMOR_POINTS_PVP: int = 100
const HEAVY_ARMOR_SPEED_MULT_PVP: float = 0.95
const MAG_PRICE_PVP: int = 100
const GRENADE_PRICE_PVP: int = 100
const UNLOCK_AK_PRICE_PVP: int = 2500
const UNLOCK_GLOCK_PRICE_PVP: int = 200
...
const WEAPON_RECOIL_PROFILES: Dictionary = {
	"glock": {
		"pattern_deg": [0.0, 0.08, 0.18, 0.34, 0.54, 0.78, 1.02, 1.18],
		"recoil_strength_mult": 3.0,
		"recoil_reset_time": 0.34,
		"recoil_randomness": 0.08,
		"move_randomness": 0.42,
		"tail_mode": "hold",
		"tail_start": 5,
		"noise_seed": 17.0
	},
	"ak": {
		"pattern_deg": [0.0, 0.10, 0.28, 0.58, 0.98, 1.55, 2.20, 2.95, 3.55, 3.20, 2.55, 1.65, 0.55, -0.55, -1.45, -2.05, -1.45, -0.35, 0.65, 1.25],
		"recoil_strength_mult": 3.0,
		"recoil_reset_time": 0.24,
		"recoil_randomness": 0.14,
		"move_randomness": 0.78,
		"tail_mode": "loop_tail",
		"tail_start": 10,
		"noise_seed": 41.0
	}
}
const ONLINE_ROUND_BUY_SEC: float = 15.0
...
const SERVER_SNAPSHOT_INTERVAL_SEC: float = 0.033
const NET_USERCMD_DT_SEC: float = 1.0 / 60.0
const SERVER_MOVE_SPEED_PVP: float = 150.0
const PKT_INPUT_BUNDLE: int = 1
const PKT_WORLD_STATE: int = 2
const PKT_SHOT_FX: int = 3
const INPUT_W: int = 1
const INPUT_S: int = 2
const INPUT_A: int = 4
const INPUT_D: int = 8
const INPUT_FIRE_HELD: int = 16
const INPUT_FIRE_JUST: int = 32
const INPUT_ALT_FIRE: int = 64
...
var server_admin_peer_id: int = 0
var server_practice_live: bool = false
var server_practice_bot_count: int = 2
var server_practice_bots_enabled: bool = true
var server_practice_bot_shooting: bool = true
var server_inf_ammo: bool = false
var server_inf_nades: bool = false
var server_inf_money: bool = false
...
func _ready() -> void:
	...
	if multiplayer.has_signal("peer_packet"):
...
func _on_peer_disconnected(id: int) -> void:
	...
	if server_admin_peer_id == id:
		server_admin_peer_id = 0
	_server_drop_peer(id)
	_server_reassign_admin_if_needed()
	_server_broadcast_runtime_flags()
...
func _on_peer_packet(peer_id: int, packet: PackedByteArray) -> void:
	if packet.is_empty():
		return
	var stream := _make_packet_stream()
	stream.data_array = packet
	var packet_type := int(stream.get_u8())
	if packet_type == PKT_INPUT_BUNDLE:
		_server_handle_input_bundle(peer_id, stream)
...
func _make_packet_stream() -> StreamPeerBuffer:
	var stream := StreamPeerBuffer.new()
	stream.big_endian = false
	return stream


func _weapon_id_to_code(weapon_id: String) -> int:
	match weapon_id:
		"ak":
			return 0
		"glock":
...
func _weapon_code_to_id(code: int) -> String:
	match code:
		0:
			return "ak"
		1:
...
func _phase_string_to_code(phase: String) -> int:
	match phase:
		"buy":
			return 1
		"live":
...
func _angle_to_u16(angle: float) -> int:
	var normalized := wrapf(angle, 0.0, TAU) / TAU
	return clampi(int(round(normalized * 65535.0)), 0, 65535)


func _u16_to_angle(value: int) -> float:
	return (float(clampi(value, 0, 65535)) / 65535.0) * TAU


func _coord_to_u16(value: float) -> int:
	var rounded := clampi(int(round(value)), -32768, 32767)
	return rounded + 32768


func _seconds_to_deci_u16(value: float) -> int:
	return clampi(int(round(maxf(0.0, value) * 10.0)), 0, 65535)


func _movement_dir_from_mask(mask: int) -> Vector2:
...
func _server_reset_recoil_state(state: Dictionary) -> void:
	for weapon_id in WEAPON_RECOIL_PROFILES.keys():
		var key := str(weapon_id)
		state["recoil_%s_index" % key] = 0
		state["recoil_%s_last" % key] = -999.0


func _server_reset_recoil_for_weapon(state: Dictionary, weapon_id: String) -> void:
	if not WEAPON_RECOIL_PROFILES.has(weapon_id):
		return
	state["recoil_%s_index" % weapon_id] = 0
	state["recoil_%s_last" % weapon_id] = -999.0


func _server_get_recoil_profile(weapon_id: String) -> Dictionary:
	var profile_v: Variant = WEAPON_RECOIL_PROFILES.get(weapon_id, {})
	if profile_v is Dictionary:
		return profile_v as Dictionary
	return {}


func _server_get_recoil_pattern_offset_deg(profile: Dictionary, shot_index: int) -> float:
	var pattern_v: Variant = profile.get("pattern_deg", [])
	if not (pattern_v is Array):
		return 0.0
	var pattern := pattern_v as Array
	if pattern.is_empty():
		return 0.0
	var idx := maxi(0, shot_index)
	if idx < pattern.size():
		return float(pattern[idx])
	var tail_mode := str(profile.get("tail_mode", "hold"))
	if tail_mode == "loop_tail":
		var tail_start := clampi(int(profile.get("tail_start", maxi(0, pattern.size() - 4))), 0, pattern.size() - 1)
		var tail_len := maxi(1, pattern.size() - tail_start)
		var local_idx := tail_start + ((idx - tail_start) % tail_len)
		return float(pattern[local_idx])
	return float(pattern[pattern.size() - 1])


func _server_get_recoil_noise_deg(profile: Dictionary, shot_index: int, is_moving: bool) -> float:
	var random_deg := maxf(0.0, float(profile.get("recoil_randomness", 0.0)))
	var move_random_deg := maxf(0.0, float(profile.get("move_randomness", 0.0)))
	var noise_amp := random_deg + (move_random_deg if is_moving else 0.0)
	if noise_amp <= 0.0:
		return 0.0
	var seed := float(profile.get("noise_seed", 1.0))
	var v: float = sin((float(shot_index) + 1.0) * (12.9898 + seed * 0.17) + seed * 7.131) * 43758.5453
	var frac: float = v - floor(v)
	return lerpf(-noise_amp, noise_amp, frac)


func _server_consume_recoil_offset_deg(state: Dictionary, weapon_id: String, shot_time_sec: float, is_moving: bool) -> float:
	var profile := _server_get_recoil_profile(weapon_id)
	if profile.is_empty():
		return 0.0
	var reset_sec := maxf(0.01, float(profile.get("recoil_reset_time", 0.2)))
	var index_key := "recoil_%s_index" % weapon_id
	var last_key := "recoil_%s_last" % weapon_id
	if shot_time_sec - float(state.get(last_key, -999.0)) > reset_sec:
		state[index_key] = 0
	var shot_index := maxi(0, int(state.get(index_key, 0)))
	var recoil_mult := maxf(0.0, float(profile.get("recoil_strength_mult", 1.0)))
	var pattern_offset := _server_get_recoil_pattern_offset_deg(profile, shot_index) * recoil_mult
	var noise_offset := _server_get_recoil_noise_deg(profile, shot_index, is_moving) * recoil_mult
	state[index_key] = shot_index + 1
	state[last_key] = shot_time_sec
	return pattern_offset + noise_offset
...
func _db_default_settings() -> Dictionary:
	return {
		"ui_language": "en",
		"mouse_sensitivity": 0.07333334,
		"minimap_corner": "right",
		"minimap_round": false,
		"crosshair_index": 0,
		"dev_disable_bots": false,
		"dev_bot_count": 3,
		"dev_inf_ammo": false,
		"dev_inf_nades": false,
		"dev_inf_money": false,
...
func _db_normalize_settings(settings_v: Variant) -> Dictionary:
	...
	if settings_v is Dictionary:
		...
		out["mouse_sensitivity"] = clampf(float(src.get("mouse_sensitivity", out["mouse_sensitivity"])), 0.005, 1.0)
		var minimap_corner := str(src.get("minimap_corner", out["minimap_corner"]))
		out["minimap_corner"] = "left" if minimap_corner == "left" else "right"
		out["minimap_round"] = bool(src.get("minimap_round", out["minimap_round"]))
		out["crosshair_index"] = clampi(int(src.get("crosshair_index", int(out["crosshair_index"]))), 0, 12)
		out["dev_disable_bots"] = bool(src.get("dev_disable_bots", out["dev_disable_bots"]))
		out["dev_bot_count"] = clampi(int(src.get("dev_bot_count", int(out["dev_bot_count"]))), 1, 10)
		out["dev_inf_ammo"] = bool(src.get("dev_inf_ammo", out["dev_inf_ammo"]))
		out["dev_inf_nades"] = bool(src.get("dev_inf_nades", out["dev_inf_nades"]))
		out["dev_inf_money"] = bool(src.get("dev_inf_money", out["dev_inf_money"]))
		out["dev_skip_buy"] = bool(src.get("dev_skip_buy", out["dev_skip_buy"]))
		out["dev_map_scale"] = clampf(float(src.get("dev_map_scale", out["dev_map_scale"])), 0.5, 4.0)
	return out
...
func _server_peer_selected_skins(peer_id: int) -> Dictionary:
	if not server_states.has(peer_id):
		return {
			"ak": "default_ak",
			"glock": "default_glock",
...
func _server_push_peer_meta(to_peer_id: int, about_peer_id: int) -> void:
	if not server_states.has(about_peer_id):
		return
	rpc_id(
		to_peer_id,
		"net_peer_meta",
		about_peer_id,
		str(server_names.get(about_peer_id, "P%d" % about_peer_id)),
		_server_peer_selected_skins(about_peer_id)
	)


func _server_push_all_peer_meta(to_peer_id: int) -> void:
	for peer_id in _server_connected_ids_sorted():
		_server_push_peer_meta(to_peer_id, peer_id)


func _server_broadcast_peer_meta(about_peer_id: int) -> void:
	if multiplayer.multiplayer_peer == null:
		return
	for peer_id in multiplayer.get_peers():
		_server_push_peer_meta(int(peer_id), about_peer_id)
	_server_push_peer_meta(about_peer_id, about_peer_id)
...
func _server_authenticated_ids_sorted() -> Array[int]:
	var ids: Array[int] = []
	for peer_id in _server_connected_ids_sorted():
		if _server_is_authenticated(peer_id):
			ids.append(peer_id)
	return ids


func _server_reassign_admin_if_needed() -> void:
	if server_admin_peer_id > 0 and _server_is_authenticated(server_admin_peer_id):
		return
	server_admin_peer_id = 0
	for peer_id in _server_authenticated_ids_sorted():
		server_admin_peer_id = peer_id
		break


func _server_is_admin(peer_id: int) -> bool:
	return peer_id > 0 and peer_id == server_admin_peer_id


func _server_team_for_peer(peer_id: int) -> String:
	var ids := _server_authenticated_ids_sorted()
	if ids.is_empty():
		return "unknown"
	if ids.size() == 1:
		return "t" if ids[0] == peer_id else "unknown"
	var idx := ids.find(peer_id)
	if idx < 0:
		return "unknown"
	return "t" if (idx % 2) == 0 else "ct"


func _server_is_practice_solo(ids: Array[int]) -> bool:
	return server_practice_live and ids.size() == 1


func _server_push_runtime_flags(peer_id: int) -> void:
	if peer_id <= 0 or not _server_is_authenticated(peer_id):
		return
	rpc_id(
		peer_id,
		"net_server_runtime_flags",
		_server_is_admin(peer_id),
		_server_team_for_peer(peer_id),
		server_practice_live,
		server_practice_bot_count,
		server_inf_ammo,
		server_inf_nades,
		server_inf_money,
		server_practice_bots_enabled,
		server_practice_bot_shooting
	)


func _server_broadcast_runtime_flags() -> void:
	for peer_id in _server_authenticated_ids_sorted():
		_server_push_runtime_flags(peer_id)


func _server_broadcast_chat_message(nick: String, team_code: String, text: String) -> void:
	for peer_id in _server_authenticated_ids_sorted():
		rpc_id(peer_id, "net_chat_message", nick, team_code, text)


func _server_broadcast_system_chat(text: String) -> void:
	for peer_id in _server_authenticated_ids_sorted():
		rpc_id(peer_id, "net_chat_message", "SYSTEM", "unknown", text)


func _server_parse_bool_arg(value: String) -> bool:
	var t := value.strip_edges().to_lower()
	return t == "1" or t == "true" or t == "on" or t == "yes"


func _server_apply_admin_command(sender: int, message: String) -> bool:
	...
	match cmd:
		"/waitskip":
			server_practice_live = true
			var ids := _server_connected_ids_sorted()
			if ids.size() == 1:
				_server_reset_peer_for_round(ids[0], 0)
				server_phase = "live"
				...
				_server_mark_dirty()
			_server_broadcast_runtime_flags()
			_server_broadcast_system_chat("Practice live enabled.")
			return true
		"/bots":
			if parts.size() < 2:
				rpc_id(sender, "net_chat_message", "SYSTEM", "unknown", "usage: /bots 0-10")
				return true
			server_practice_bot_count = clampi(int(parts[1]), 0, 10)
			if server_practice_bot_count > 0:
				server_practice_bots_enabled = true
			_server_broadcast_runtime_flags()
			_server_broadcast_system_chat("Practice bots set to %d." % server_practice_bot_count)
			return true
		"/botspawn":
			if parts.size() < 2:
				rpc_id(sender, "net_chat_message", "SYSTEM", "unknown", "usage: /botspawn 0-10")
				return true
			server_practice_bot_count = clampi(int(parts[1]), 0, 10)
			if server_practice_bot_count > 0:
				server_practice_bots_enabled = true
			_server_broadcast_runtime_flags()
			_server_broadcast_system_chat("Bot spawn count set to %d." % server_practice_bot_count)
			return true
		"/botstart":
			if server_practice_bot_count <= 0:
				server_practice_bot_count = 2
			server_practice_bots_enabled = true
			_server_broadcast_runtime_flags()
			_server_broadcast_system_chat("Practice bots ON.")
			return true
		"/botstop":
			server_practice_bots_enabled = false
			_server_broadcast_runtime_flags()
			_server_broadcast_system_chat("Practice bots OFF.")
			return true
		"/botshoot":
			if parts.size() < 2:
				rpc_id(sender, "net_chat_message", "SYSTEM", "unknown", "usage: /botshoot 0/1")
				return true
			server_practice_bot_shooting = _server_parse_bool_arg(parts[1])
			_server_broadcast_runtime_flags()
			_server_broadcast_system_chat("Practice bot shooting %s." % ("ON" if server_practice_bot_shooting else "OFF"))
			return true
		"/botshootstart":
			server_practice_bot_shooting = true
			_server_broadcast_runtime_flags()
			_server_broadcast_system_chat("Practice bot shooting ON.")
			return true
		"/botshootstop":
			server_practice_bot_shooting = false
			_server_broadcast_runtime_flags()
			_server_broadcast_system_chat("Practice bot shooting OFF.")
			return true
		"/infammo":
			if parts.size() < 2:
				rpc_id(sender, "net_chat_message", "SYSTEM", "unknown", "usage: /infammo 0/1")
				return true
			server_inf_ammo = _server_parse_bool_arg(parts[1])
			_server_broadcast_runtime_flags()
			_server_broadcast_system_chat("Infinite ammo %s." % ("ON" if server_inf_ammo else "OFF"))
			return true
		"/infnades":
			if parts.size() < 2:
				rpc_id(sender, "net_chat_message", "SYSTEM", "unknown", "usage: /infnades 0/1")
				return true
			server_inf_nades = _server_parse_bool_arg(parts[1])
			_server_broadcast_runtime_flags()
			_server_broadcast_system_chat("Infinite grenades %s." % ("ON" if server_inf_nades else "OFF"))
			return true
		"/infmoney":
			if parts.size() < 2:
				rpc_id(sender, "net_chat_message", "SYSTEM", "unknown", "usage: /infmoney 0/1")
				return true
			server_inf_money = _server_parse_bool_arg(parts[1])
			_server_broadcast_runtime_flags()
			_server_broadcast_system_chat("Infinite money %s." % ("ON" if server_inf_money else "OFF"))
			return true
		_:
			rpc_id(sender, "net_chat_message", "SYSTEM", "unknown", "unknown command")
			return true


func _server_handle_input_bundle(peer_id: int, stream: StreamPeerBuffer) -> void:
	if not _server_is_authenticated(peer_id):
		return
	_server_ensure_peer(peer_id)
	if not server_states.has(peer_id):
		return
	var cmd_count := clampi(int(stream.get_u8()), 0, 24)
	if cmd_count <= 0:
		return
	var now_sec := _server_now_sec()
	var base_time := now_sec - float(maxi(0, cmd_count - 1)) * NET_USERCMD_DT_SEC
	for i in range(cmd_count):
		var cmd := {
			"seq": int(stream.get_u32()),
			"mask": int(stream.get_u8()),
			"aim": _u16_to_angle(int(stream.get_u16())),
			"weapon": _weapon_code_to_id(int(stream.get_u8())),
			"grenade_index": int(stream.get_u8()),
			"ak_auto": int(stream.get_u8()) != 0
		}
		_server_apply_input_command(peer_id, cmd, base_time + float(i) * NET_USERCMD_DT_SEC)


func _server_apply_input_command(peer_id: int, cmd: Dictionary, cmd_time_sec: float) -> void:
	if not server_states.has(peer_id):
		return
	var s: Dictionary = server_states[peer_id]
	var seq := int(cmd.get("seq", 0))
	var last_seq := int(s.get("last_seq", 0))
	if seq <= last_seq:
		return
	if _server_update_reloads_for_state(s, cmd_time_sec):
		server_states[peer_id] = s
	var prev_weapon := str(s.get("weapon", "glock"))
	var weapon_id := str(cmd.get("weapon", s.get("weapon", "glock")))
	if not _server_weapon_owned(s, weapon_id):
		weapon_id = "glock"
	if not (weapon_id == "ak" or weapon_id == "glock" or weapon_id == "knife" or weapon_id == "grenade"):
		weapon_id = "glock"
	if weapon_id != prev_weapon:
		if prev_weapon == "ak":
			s["ak_reload_end"] = 0.0
		elif prev_weapon == "glock":
			s["glock_reload_end"] = 0.0
		_server_reset_recoil_for_weapon(s, prev_weapon)
		_server_reset_recoil_for_weapon(s, weapon_id)
	s["weapon"] = weapon_id
	s["aim"] = float(cmd.get("aim", s.get("aim", 0.0)))
	s["ak_auto"] = bool(cmd.get("ak_auto", s.get("ak_auto", true)))
	var mask := int(cmd.get("mask", 0))
	if bool(s.get("alive", true)):
		var practice_solo := _server_is_practice_solo(_server_connected_ids_sorted())
		var allow_move := server_phase == "live" or server_phase == "waiting" or (server_phase == "buy" and practice_solo)
		var is_moving := false
		if allow_move:
			var dir := _movement_dir_from_mask(mask)
			is_moving = dir.length_squared() > 0.0
			var speed_mult := HEAVY_ARMOR_SPEED_MULT_PVP if str(s.get("armor_type", "none")) == "heavy" and int(s.get("armor", 0)) > 0 else 1.0
			var move_speed := SERVER_MOVE_SPEED_PVP * speed_mult
			s["x"] = float(s.get("x", 0.0)) + dir.x * move_speed * NET_USERCMD_DT_SEC
			s["y"] = float(s.get("y", 0.0)) + dir.y * move_speed * NET_USERCMD_DT_SEC
		if weapon_id == "ak":
			var ak_auto := bool(s.get("ak_auto", true))
			if (ak_auto and (mask & INPUT_FIRE_HELD) != 0) or ((not ak_auto) and (mask & INPUT_FIRE_JUST) != 0):
				_server_process_hitscan_shot(peer_id, weapon_id, s, cmd_time_sec, is_moving)
		elif weapon_id == "glock" and (mask & INPUT_FIRE_JUST) != 0:
			_server_process_hitscan_shot(peer_id, weapon_id, s, cmd_time_sec, is_moving)
		elif weapon_id == "knife":
			if (mask & INPUT_ALT_FIRE) != 0:
				_server_process_knife_attack(peer_id, s, cmd_time_sec, true)
			elif (mask & INPUT_FIRE_HELD) != 0 or (mask & INPUT_FIRE_JUST) != 0:
				_server_process_knife_attack(peer_id, s, cmd_time_sec, false)
	s["last_seq"] = seq
	server_states[peer_id] = s
	_server_mark_dirty()
...
func _server_start_buy_phase() -> void:
	var ids := _server_connected_ids_sorted()
	if ids.size() < 2 and not _server_is_practice_solo(ids):
...
func _server_end_round(winner_id: int) -> void:
	for peer_id in _server_connected_ids_sorted():
		if not server_states.has(peer_id):
			continue
		var round_state: Dictionary = server_states[peer_id]
		var round_reward := 3000
		if winner_id > 0 and peer_id == winner_id:
			round_reward = 4500
		round_state["money"] = int(round_state.get("money", START_MONEY_PVP)) + round_reward
		server_states[peer_id] = round_state
	if winner_id > 0:
...
func _server_tick_round(delta: float) -> void:
	var ids := _server_connected_ids_sorted()
	var practice_solo := _server_is_practice_solo(ids)
	...
	if ids.size() < 2 and not practice_solo:
		server_phase = "waiting"
		if ids.size() == 1:
			_server_ensure_peer(ids[0])
			if server_states.has(ids[0]):
				var lone_state: Dictionary = server_states[ids[0]]
				if not bool(lone_state.get("alive", false)) or int(lone_state.get("hp", 0)) <= 0:
					_server_reset_peer_for_round(ids[0], 0)
		...
		_server_mark_dirty()
		return
	if practice_solo and server_phase == "waiting":
		_server_reset_peer_for_round(ids[0], 0)
	...
	match server_phase:
		...
		"live":
			...
			if not practice_solo and alive_ids.size() <= 1:
				_server_end_round(alive_ids[0] if alive_ids.size() == 1 else -1)
			elif server_phase_time_left <= 0.0:
				_server_end_round(alive_ids[0] if practice_solo and alive_ids.size() == 1 else -1)
		"post_round":
			if server_phase_time_left <= 0.0:
				_server_start_buy_phase()
		"match_over":
...
func _server_ensure_peer(peer_id: int) -> void:
	...
	var can_spawn_now := server_phase == "waiting" or server_phase == "buy" or server_phase == "live"
	var state := {
		"x": spawn.x,
		"y": spawn.y,
		"aim": 0.0,
	...
	_server_reset_recoil_state(state)
	server_states[peer_id] = state
...
func _server_apply_buy(peer_id: int, action_id: String) -> void:
	...
	if not server_states.has(peer_id):
		return
	if server_phase != "buy" and server_phase != "waiting":
		return
	var s: Dictionary = server_states[peer_id]
	if not bool(s.get("alive", true)):
		return
	var money_now := int(s.get("money", START_MONEY_PVP))
	var free_buy := server_phase == "waiting" or server_inf_money
	match action_id:
		"armor", "armor_heavy":
			if (free_buy or money_now >= HEAVY_ARMOR_PRICE_PVP) and int(s.get("armor", 0)) < HEAVY_ARMOR_POINTS_PVP:
				if not free_buy:
					money_now -= HEAVY_ARMOR_PRICE_PVP
				s["armor"] = HEAVY_ARMOR_POINTS_PVP
				s["armor_type"] = "heavy"
		"armor_light":
			if (free_buy or money_now >= LIGHT_ARMOR_PRICE_PVP) and int(s.get("armor", 0)) < LIGHT_ARMOR_POINTS_PVP:
				if not free_buy:
					money_now -= LIGHT_ARMOR_PRICE_PVP
				s["armor"] = LIGHT_ARMOR_POINTS_PVP
				s["armor_type"] = "light"
		"grenade_palak", "grenade_smoke", "grenade_flash", "grenade_gas", "grenade_he":
			if free_buy or money_now >= GRENADE_PRICE_PVP:
				if not free_buy:
					money_now -= GRENADE_PRICE_PVP
		"mag_ak":
			var mags_ak := int(s.get("ak_mags", 0))
			if (free_buy or money_now >= MAG_PRICE_PVP) and mags_ak < MAX_SPARE_MAGS_PER_WEAPON_PVP:
				if not free_buy:
					money_now -= MAG_PRICE_PVP
				s["ak_mags"] = mags_ak + 1
		"mag_glock":
			var mags_g := int(s.get("glock_mags", 1))
			if (free_buy or money_now >= MAG_PRICE_PVP) and mags_g < MAX_SPARE_MAGS_PER_WEAPON_PVP:
				if not free_buy:
					money_now -= MAG_PRICE_PVP
				s["glock_mags"] = mags_g + 1
		"unlock_ak":
			if (not bool(s.get("ak_owned", false))) and (free_buy or money_now >= UNLOCK_AK_PRICE_PVP):
				if not free_buy:
					money_now -= UNLOCK_AK_PRICE_PVP
				s["ak_owned"] = true
		"unlock_glock":
			if (not bool(s.get("glock_owned", true))) and (free_buy or money_now >= UNLOCK_GLOCK_PRICE_PVP):
				if not free_buy:
...
func _server_apply_reload(peer_id: int, weapon_id: String) -> void:
	...
	if not bool(s.get("alive", true)):
		return
	var practice_solo := _server_is_practice_solo(_server_connected_ids_sorted())
	if server_phase != "live" and server_phase != "waiting" and not (server_phase == "buy" and practice_solo):
		return
	var now_sec := _server_now_sec()
...
func _server_update_state(peer_id: int, state: Dictionary) -> void:
	...
	if w != prev_weapon:
		if prev_weapon == "ak":
			...
			s["glock_reload_end"] = 0.0
		_server_reset_recoil_for_weapon(s, prev_weapon)
		_server_reset_recoil_for_weapon(s, w)
	s["weapon"] = w
	server_states[peer_id] = s
	_server_mark_dirty()
...
func _server_build_world_packet() -> PackedByteArray:
	var stream := _make_packet_stream()
	var now_sec := _server_now_sec()
	stream.put_u8(PKT_WORLD_STATE)
	stream.put_u8(_phase_string_to_code(server_phase))
	stream.put_u16(_seconds_to_deci_u16(server_phase_time_left))
	stream.put_u16(_seconds_to_deci_u16(server_wait_time_left if server_phase == "waiting" else 0.0))
	stream.put_u32(server_match_winner)
	stream.put_u8(server_states.size())
	var ids := _server_connected_ids_sorted()
	for peer_id in ids:
		var state_ref: Dictionary = server_states[peer_id]
		if _server_update_reloads_for_state(state_ref, now_sec):
		...
		if bool(s.get("alive", true)):
			flags |= 1
		flags |= (_weapon_id_to_code(str(s.get("weapon", "glock"))) & 7) << 1
		if bool(s.get("ak_owned", false)):
			flags |= 16
		if bool(s.get("glock_owned", true)):
			flags |= 32
		stream.put_u32(peer_id)
		stream.put_u32(int(s.get("last_seq", 0)))
		stream.put_u8(flags)
		stream.put_u16(_coord_to_u16(float(s.get("x", 0.0))))
		stream.put_u16(_coord_to_u16(float(s.get("y", 0.0))))
		stream.put_u16(_angle_to_u16(float(s.get("aim", 0.0))))
		stream.put_u8(clampi(int(s.get("hp", SERVER_MAX_HP)), 0, 255))
		stream.put_u8(clampi(int(s.get("armor", 0)), 0, 255))
		stream.put_u16(clampi(int(s.get("money", START_MONEY_PVP)), 0, 65535))
		stream.put_u8(clampi(int(s.get("ak_clip", AK_CLIP_SIZE_PVP)), 0, 255))
		stream.put_u8(clampi(int(s.get("ak_mags", 0)), 0, 255))
		stream.put_u8(clampi(int(s.get("glock_clip", GLOCK_CLIP_SIZE_PVP)), 0, 255))
		stream.put_u8(clampi(int(s.get("glock_mags", 1)), 0, 255))
		stream.put_u16(_seconds_to_deci_u16(maxf(0.0, float(s.get("ak_reload_end", 0.0)) - now_sec)))
		stream.put_u16(_seconds_to_deci_u16(maxf(0.0, float(s.get("glock_reload_end", 0.0)) - now_sec)))
		stream.put_u8(clampi(int(server_scores.get(peer_id, 0)), 0, 255))
		stream.put_u8(clampi(enemy_score_best, 0, 255))
		stream.put_u8(clampi(opponents, 0, 255))
	return stream.data_array


func _server_broadcast_shot_fx(peer_id: int, weapon: String, shot_angle: float, length_px: float) -> void:
	if multiplayer.multiplayer_peer == null:
		return
	var stream := _make_packet_stream()
	stream.put_u8(PKT_SHOT_FX)
	stream.put_u32(peer_id)
	stream.put_u8(_weapon_id_to_code(weapon))
	stream.put_u16(_angle_to_u16(shot_angle))
	stream.put_u16(clampi(int(round(length_px)), 0, 65535))
	multiplayer.send_bytes(stream.data_array, 0, MultiplayerPeer.TRANSFER_MODE_UNRELIABLE_ORDERED)


func _server_broadcast_state() -> void:
	if multiplayer.multiplayer_peer == null:
		return
	multiplayer.send_bytes(_server_build_world_packet(), 0, MultiplayerPeer.TRANSFER_MODE_UNRELIABLE_ORDERED)
...
func _server_process_knife_attack(sender: int, shooter_state: Dictionary, shot_time_sec: float, is_heavy: bool) -> void:
	if server_phase != "live" and server_phase != "waiting":
		return
	if not bool(shooter_state.get("alive", true)):
		return
	var next_fire_time := float(shooter_state.get("next_fire_time", 0.0))
	if shot_time_sec < next_fire_time:
		return
	shooter_state["next_fire_time"] = shot_time_sec + (0.74 if is_heavy else 0.32)
	server_states[sender] = shooter_state
	var damage := 58 if is_heavy else 26
	var melee_range := 68.0
	var aim_angle := float(shooter_state.get("aim", 0.0))
	var origin := Vector2(float(shooter_state.get("x", 0.0)), float(shooter_state.get("y", 0.0)))
	...
	for peer_id in server_states.keys():
		...
		if along < 0.0 or along > melee_range:
			continue
		var dist := _distance_point_to_ray(origin, dir, target_pos, melee_range)
		if dist <= HIT_RADIUS_PX and along < best_along:
			best_along = along
	...
	var absorbed := 0
	if armor_now > 0:
		absorbed = mini(armor_now, int(round(float(damage) * 0.6)))
		armor_now -= absorbed
	var hp_loss := maxi(0, damage - absorbed)
	var hp_now := int(v.get("hp", SERVER_MAX_HP)) - hp_loss
	if hp_now <= 0:
		hp_now = 0
		armor_now = 0
		v["alive"] = false
	...
	if hp_now <= 0:
		shooter_state["money"] = int(shooter_state.get("money", START_MONEY_PVP)) + KILL_REWARD_PVP
		server_states[sender] = shooter_state


func _server_process_hitscan_shot(sender: int, weapon: String, shooter_state: Dictionary, shot_time_sec: float, is_moving: bool = false) -> void:
	var practice_solo := _server_is_practice_solo(_server_connected_ids_sorted())
	if server_phase != "live" and server_phase != "waiting" and not (server_phase == "buy" and practice_solo):
		return
	...
	if not _server_weapon_owned(shooter_state, weapon):
		return
	if _server_is_weapon_reloading(shooter_state, weapon, shot_time_sec):
		return
	var next_fire_time := float(shooter_state.get("next_fire_time", 0.0))
	if shot_time_sec < next_fire_time:
		return
	var fire_rate := 10.0 if weapon == "ak" else 6.0
	shooter_state["next_fire_time"] = shot_time_sec + (1.0 / fire_rate)
	if weapon == "ak":
		var ak_clip_now := int(shooter_state.get("ak_clip", AK_CLIP_SIZE_PVP))
		if ak_clip_now <= 0 and not server_inf_ammo:
			return
		if not server_inf_ammo:
			var ak_after := ak_clip_now - 1
			shooter_state["ak_clip"] = ak_after
			if ak_after <= 0 and int(shooter_state.get("ak_mags", 0)) > 0 and not _server_is_weapon_reloading(shooter_state, "ak", shot_time_sec):
				shooter_state["ak_reload_end"] = shot_time_sec + AK_RELOAD_SEC_PVP
	else:
		var glock_clip_now := int(shooter_state.get("glock_clip", GLOCK_CLIP_SIZE_PVP))
		if glock_clip_now <= 0 and not server_inf_ammo:
			return
		if not server_inf_ammo:
			var glock_after := glock_clip_now - 1
			shooter_state["glock_clip"] = glock_after
			if glock_after <= 0 and int(shooter_state.get("glock_mags", 1)) > 0 and not _server_is_weapon_reloading(shooter_state, "glock", shot_time_sec):
				shooter_state["glock_reload_end"] = shot_time_sec + GLOCK_RELOAD_SEC_PVP
	server_states[sender] = shooter_state
	var shooter_pos := Vector2(float(shooter_state.get("x", 0.0)), float(shooter_state.get("y", 0.0)))
	var shot_angle := float(shooter_state.get("aim", 0.0)) + deg_to_rad(_server_consume_recoil_offset_deg(shooter_state, weapon, shot_time_sec, is_moving))
	server_states[sender] = shooter_state
	var dir := Vector2.RIGHT.rotated(shot_angle)
	var max_range := float(WEAPON_RANGE.get(weapon, 1000.0))
	var victim_id: int = -1
	var best_dist_along: float = max_range
	for peer_id in server_states.keys():
		if int(peer_id) == sender:
			continue
		var target_state: Dictionary = server_states[peer_id]
	...
	var hit_len := best_dist_along if victim_id != -1 else max_range
	_server_broadcast_shot_fx(sender, weapon, shot_angle, hit_len)
	_server_mark_dirty()
...
func net_submit_profile(profile: Dictionary) -> void:
	...
	if login != "":
		...
		if not user.is_empty():
			...
			_db_set_user(login, user)
			_db_save()
	_server_broadcast_peer_meta(sender)
	_server_mark_dirty()
...
func net_submit_shot(payload: Dictionary) -> void:
	...
	var s: Dictionary = server_states.get(sender, {})
	if s.is_empty():
		return
	if not payload.is_empty():
		if payload.has("aim"):
			s["aim"] = float(payload.get("aim", s.get("aim", 0.0)))
		elif payload.has("dx") and payload.has("dy"):
			var legacy_dir := Vector2(float(payload.get("dx", 0.0)), float(payload.get("dy", 0.0)))
			if legacy_dir.length_squared() > 0.0001:
				s["aim"] = legacy_dir.angle()
		s["weapon"] = str(payload.get("weapon", s.get("weapon", "glock")))
	_server_process_hitscan_shot(sender, str(s.get("weapon", "glock")), s, Time.get_ticks_msec() / 1000.0, bool(payload.get("moving", false)))
...
@rpc("any_peer")
func net_submit_chat(message: String) -> void:
	var sender := multiplayer.get_remote_sender_id()
	if not _server_is_authenticated(sender):
		return
	...
	if clean == "":
		return
	if clean.length() > 180:
		clean = clean.substr(0, 180)
	if clean.begins_with("/") and _server_is_admin(sender):
		if _server_apply_admin_command(sender, clean):
			return
	var nick := str(server_names.get(sender, "P%d" % sender))
	_server_broadcast_chat_message(nick, _server_team_for_peer(sender), clean)
...
func net_submit_auth(payload: Dictionary) -> void:
	...
	if server_admin_peer_id <= 0:
		server_admin_peer_id = sender
	_server_ensure_peer(sender)
	var final_nick := nick
	if final_nick == "":
	...
	_server_push_all_peer_meta(sender)
	_server_broadcast_peer_meta(sender)
	_server_reassign_admin_if_needed()
	_server_broadcast_runtime_flags()
	_server_mark_dirty()
...
@rpc("authority", "call_local")
func net_peer_meta(_peer_id: int, _nick: String, _selected_skins: Dictionary) -> void:
	pass
...
@rpc("authority", "call_local")
func net_chat_message(_nick: String, _team_code: String, _message: String) -> void:
	pass


@rpc("authority", "call_local")
func net_server_runtime_flags(_is_admin_flag: bool, _team_code: String, _practice_mode: bool, _practice_bot_count: int, _inf_ammo: bool, _inf_nades: bool, _inf_money: bool, _practice_bots_enabled: bool = true, _practice_bot_shooting: bool = true) -> void:
	pass
